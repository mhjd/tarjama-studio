...
web-review-prepare:
	python3 web/review/prepare.py --base "$(REVIEW_BASE)" --review-image "$(REVIEW_IMAGE)" --recorder-image "$(RECORDER_IMAGE)" --run "$(REVIEW_RUN)" --output "$(REVIEW_RECIPE)" --video "$(REVIEW_VIDEO)" $(if $(REVIEW_DEVICE),--device "$(REVIEW_DEVICE)") $(if $(filter 1,$(REVIEW_RESUME)),--resume,) $(if $(REVIEW_ARTIFACT_PREFIX),--artifact-prefix "$(REVIEW_ARTIFACT_PREFIX)")
