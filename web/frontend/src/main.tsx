...
import { Drafts, purgeDrafts } from "./drafts";
import "./styles.css";
function App() {
...
function Create({
  ...
  async function create() {
    ...
    try {
      ...
      initialFile = mode === "file" ? file : null;
      onCreate(p);
    } catch (e) {
      setError(String(e));
    } finally {
...
let initialFile: File | null = null;
function Editor({
  initial,
  user,
  back,
    ...
    <>
      ...
      {p.segments.length > 0 && (
        <>
          ...
          {p.stage === "arabic" && p.translation_source > 0 && (
            <p className="notice">
      ...
      {p.stage === "arabic" && (
        <section className="next-step">
          ...
          <button
            className="primary"
            disabled={busy || (p.translation_source > 0 && !replace)}
            onClick={() =>
              void action(async () => {
                d.adopt(
      ...
      {p.stage === "ready" && (
        <section className="panel">
          ...
          {jobs
            .filter(
              (j) => j.kind.startsWith("export_") && j.state === "succeeded",
            )
            .map((j) => (
              <p key={j.id}>
...
function Keys({ close }: { close: () => void }) {
