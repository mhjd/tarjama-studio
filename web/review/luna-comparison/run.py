...
def request_body(model, prompt, source, capability=False, context_source=(), api='chat', reasoning='none', continuity=False, previous_summary='', provider=None, expected_provider=None, json_object=False, max_completion_price=3, no_output_limit=False):
...
def run_call(out, model, case, prompt, source, key, capability=False, context_source=(), api='chat', reasoning='none', timeout_seconds=300, continuity=False, previous_summary='', provider=None, expected_provider=None, json_object=False, max_completion_price=3, no_output_limit=False):
    folder = out / (case + '-' + model.split('/')[1]); folder.mkdir(mode=0o700)
    save(folder / 'input.json', source)
    body = request_body(model, prompt, source, capability, context_source, api, reasoning, continuity, previous_summary, provider, expected_provider, json_object, max_completion_price, no_output_limit)
...
def main():
    ...
    parser.add_argument('--no-output-limit', action='store_true', help='Use provider/model output defaults without a caller token ceiling')
    ...
    save(out / 'protocol.json', {'models': target_models, 'max_http_calls': (len(timed) if args.continuity else 1 if args.case else len(timed)+3 if args.chunk_minutes else 4) * len(target_models), 'retries': 0,
        ...
        'json_object': args.json_object, 'no_output_limit': args.no_output_limit, 'provider_fallbacks': False, 'provider_only': args.provider, 'expected_provider': expected_provider, 'max_price_per_million_usd': {'prompt': 1, 'completion': args.max_completion_price},
    ...
    for index, (case, fixture) in enumerate(cases):
        ...
        with ProcessPoolExecutor(max_workers=len(target_models)) as pool:
            futures = [pool.submit(run_call, out, model, case, prompt if fixture else capability_prompt,
                        source, key, not fixture, context_source, args.api, args.reasoning, args.timeout_seconds, args.continuity, previous_summary, args.provider, expected_provider, args.json_object, args.max_completion_price, args.no_output_limit) for model in order]
